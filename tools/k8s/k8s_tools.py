...
@mcp.tool()
def get_pod_logs(
    pod_name, namespace, container=None, previous=False, tail_lines=None
):
    """
    Retrieves logs for a specified pod and container.

    Args:
        pod_name (str): Name of the pod.
        namespace (str): Namespace where the pod is running.
        container (str, optional): Name of the container within the pod.
            If None, and the pod has only one container, its logs will be fetched.
            If the pod has multiple containers, this argument is required.
        previous (bool, optional): If True, return logs from the previous
            instance of the container. Defaults to False.
        tail_lines (int, optional): Number of lines from the end of the logs to
            retrieve. If None, all lines are returned. Defaults to None.

    Returns:
        str: Logs from the container, or a JSON string with an error message.
    ...
    try:
        ...
        # Remove None values so default Kubernetes client behavior is used
        log_options = {k: v for k, v in log_options.items() if v is not None}

        logs = core_v1.read_namespaced_pod_log(
            name=pod_name, namespace=namespace, **log_options
        )
        return logs
    except ApiException as e:
        # Try to provide a more specific error if the container name is missing for a multi-container pod
        if "a container name must be specified for pod" in str(e) and not container:
            try:
                pod = core_v1.read_namespaced_pod(name=pod_name, namespace=namespace)
                container_names = [c.name for c in pod.spec.containers]
                return json.dumps({
                    "error": f"Pod '{pod_name}' has multiple containers: {container_names}. Please specify a container name."
                }), 400
            except Exception: # Fallback to original error
                 return json.dumps({"error": str(e), "details": "Error fetching container names for multi-container pod."}), 500
        return json.dumps({"error": str(e)}), 500
